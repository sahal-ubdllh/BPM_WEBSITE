// pages.js — shared for all sub-pages
...
function getH() { return header ? header.offsetHeight : 72; }
function setPos() {
    if (window.innerWidth <= 768) {
        navLinks.style.top    = getH() + 'px';
        navLinks.style.height = `calc(100dvh - ${getH()}px)`;
    }
}
function openMenu() {
    setPos();
    ...
    navItems.forEach((item,i) => {
        ...
        item.style.animation = `navItemFade .4s ease forwards ${i*.07+.05}s`;
    });
}
function closeMenu() {
...
if (burger) burger.addEventListener('click', () => navLinks.classList.contains('nav-active') ? closeMenu() : openMenu());
if (dropdown) {
    dropdown.querySelector('.dropbtn').addEventListener('click', e => {
        if (window.innerWidth <= 768) { e.preventDefault(); dropdown.classList.toggle('dropdown-active'); }
    });
}
document.addEventListener('click', e => {
    if (!burger) return;
    if (!navLinks.contains(e.target) && !burger.contains(e.target) && navLinks.classList.contains('nav-active')) closeMenu();
});
window.addEventListener('resize', () => {
    if (window.innerWidth > 768) { closeMenu(); navLinks.style.top=''; navLinks.style.height=''; }
    else if (navLinks.classList.contains('nav-active')) setPos();
});
window.addEventListener('scroll', () => { if (navLinks.classList.contains('nav-active')) setPos(); }, {passive:true});

// Scroll reveal
const obs = new IntersectionObserver(entries => {
    entries.forEach((e,i) => {
        if (e.isIntersecting) {
            e.target.style.animation = `revealUp .55s ease ${i*.08}s both`;
            obs.unobserve(e.target);
        }
    });
}, {threshold:.1});
document.querySelectorAll('.reveal').forEach(el => { el.style.opacity='0'; obs.observe(el); });
